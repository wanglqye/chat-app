	...
	<view class="emotion-box">
			<swiper class="swiper" disable-programmatic-animation="true" indicator-dots="true">
				<swiper-item class="emotion-box-line">
					<view v-for="(line, i) in state.list1" v-bind:key="i">
						<view v-for="(im, ix) in line" v-bind:key="ix" class="swiper-item emotion-item" :style="{ width: state.img_width + 'px' }" @click="pops(im)">
							<rich-text :nodes="im.toString().replace(/^([1-9]\d?|97)$/g, emotion(im))"></rich-text>
						</view>
					</view>
				</swiper-item>
				<swiper-item class="emotion-box-line">
					<view v-for="(line, i) in state.list2" v-bind:key="i">
						<view v-for="(im, ix) in line" v-bind:key="ix" class="swiper-item emotion-item" :style="{ width: state.img_width + 'px' }" @click="clickHandler(im)">
							<rich-text :nodes="im.toString().replace(/^([1-9]\d?|97)$/g, emotion(im))"></rich-text>
						</view>
					</view>
				</swiper-item>
				<swiper-item class="emotion-box-line">
					<view v-for="(line, i) in state.list3" v-bind:key="i">
						<view v-for="(im, ix) in line" v-bind:key="ix" class="swiper-item emotion-item" :style="{ width: state.img_width + 'px' }" @click="clickHandler(im)">
							<rich-text :nodes="im.toString().replace(/^([1-9]\d?|97)$/g, emotion(im))"></rich-text>
						</view>
					</view>
				...
				<swiper-item class="emotion-box-line">
					<view v-for="(line, i) in state.list4" v-bind:key="i">
						<view v-for="(im, ix) in line" v-bind:key="ix" class="swiper-item emotion-item" :style="{ width: state.img_width + 'px' }" @click="clickHandler(im)">
							<rich-text :nodes="im.toString().replace(/^([1-9]\d?|97)$/g, emotion(im))"></rich-text>
						</view>
					</view>
...
<script setup>
	...
	function emotion(res){
		...
		// 服务器地址
		return  `<img src="http://localhost:3008/emoji/${index}.gif" align="middle">`
		// return  `<img src="../../static/emoji/${index}.gif" align="middle">`
